(function () {
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    document.getElementById("authForm")?.addEventListener("submit",async e=>{e.preventDefault();message("");if(!configured)return message("La connexion cloud n’est pas encore configurée.");const button=e.submitter;button.disabled=true;button.textContent="Connexion…";const email=document.getElementById("authEmail").value,password=document.getElementById("authPassword").value;const {error}=await client.auth.signInWithPassword({email,password});button.disabled=false;button.textContent="Se connecter";if(error)message("Connexion impossible. Vérifiez vos informations.")});
